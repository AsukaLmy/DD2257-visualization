...
protected:
    ...
    void extractCriticalPoints(const VectorField2& vectorField, std::vector<dvec2>& criticalpointlist, int depth, int maxdepth, double x, double y, double boxlength_x, double boxlength_y,
                                      double lengththreshold);
    bool zerodetection(const VectorField2& vectorField, double x, double y, double boxlength_x, double boxlength_y);
    void modefyList(std::vector<dvec2>& criticalpointlist, double threshold);
    void boundaryswitchpoint( const VectorField2& vectorField,std::vector<dvec2>& boundarypointlist, double lb_x, double lb_y, double rt_x, double rt_y, double eps);
...
public:
    ...
    // Properties
public:
    IntProperty Maxdepth;
    DoubleProperty lengththreshold;
    DoubleProperty modefythreshold;
    DoubleProperty eigenstep;
